...
def get_features(sample):
    ...
    y = sample.shape[1]
    z = sample.shape[2]
    frame_feature = np.empty([x,y,z])
    for i in range(0,z):
        frame = sample[:,:,i]
        hip = sample[0,:,i]
        for j in range(0,x):
            frame_feature[j,:,i] = frame[j,:] - hip
    feature_all = np.concatenate((sample, frame_feature), axis=1)
    feature = np.transpose(feature_all, (1,2,0))
    ...
    def next_batch(self, batch_size):
        ...
        for vid in batch:
            try:
                ...
                print('stop')
            try:
                file_ptr = np.load(self.gt_path + vid)
            except ValueError:
                print('stop')
        ...
        batch_input_tensor = torch.zeros(len(batch_input), 6, max(length_of_sequences), 17, 1, dtype=torch.float)
