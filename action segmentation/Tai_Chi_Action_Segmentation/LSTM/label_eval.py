...
def mcc_score(recognized, ground_truth, overlap, bg_class=[0]):
    p_label, p_start, p_end = get_labels_start_end_time(recognized, bg_class)
    y_label, y_start, y_end = get_labels_start_end_time(ground_truth, bg_class)
...
def run_eval(path):
    ...
    print(args.dataset)
    ground_truth_path = "./data/" + args.dataset + "/label/"
    recog_path = "./results/"+args.dataset+"/"+ path +"/"
    file_list = "./data/" + args.dataset + "/split1/test" + ".txt"
    ...
    for vid in list_of_videos:
        ...
        gt_content = np.load(gt_file).astype(int).tolist()[0]
        gt_content = gt_content[::2].copy()  
    ...
    for s in range(len(overlap)):
        ...
        mcc = ((tp2[s]*tn2[s])-(fn2[s]*fp2[s]))/(math.sqrt((tp2[s]+fp2[s])*(tp2[s]+fn2[s])*(tn2[s]+fp2[s])*(tn2[s]+fn2[s]))+0.0000001) 
